...
assert.match(elements["#project-detail"].innerHTML, /推进项/);
evaluate('state.projects = []; state.tasks = [{id: 9, title: "Legacy item", status: "todo", priority: "medium"}]; state.selectedProjectId = null; renderTasks()');
assert.match(elements["#task-board"].innerHTML, /未归类推进项/);
assert.match(elements["#project-detail"].innerHTML, /Legacy item/);
